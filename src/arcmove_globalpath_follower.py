...
def arcmove(ox, oy, oth, tx, ty, tth, gth):
	...
	global initialturn, waitonaboutface
	...
	arclength = 0
	goalrotate = False	
	if followpath and not ox==tx and not oy==ty: # normal arc move
		# distance > 0, determine arc length here
		distance = math.sqrt( pow((tx - ox),2) + pow((ty - oy),2) )
		dth = tth - oth
		radius = (distance/2)/math.sin(dth/2)
		arclength = radius * dth # *should* work out to always be > 0
	elif goalpose:  # final goal rotate move
		dth = (gth - tfth) - oth # goal pose is in tf coordinate space!
		goalrotate = True
	...
	# turn in place if > 70 deg
	if arclength > 0 and abs(dth) > 1.22:   
		initialturn = False
		arclength = 0
