...
listener = None
GOALTHRESHOLD = 0.9 # meters from goal
...
def globalPathCallback(data):
	...
	if n-1 < GLOBALPATHPOSENUM or goalDistance() <= GOALTHRESHOLD:
		p = data.poses[n-1] 
	else:
...
def arcmove(ox, oy, oth, gpx, gpy, gpth, gth, lpx, lpy, lpth):
	...
	if abs(dth) > 1.57 and not goalrotate and not initialturn and waitonaboutface < 1 and arclength > 0: 
		if goalDistance() > GOALTHRESHOLD: # skip if close to goal
			# oculusprimesocket.clearIncoming()
			# oculusprimesocket.sendString("forward 0.25")
			# oculusprimesocket.waitForReplySearch("<state> direction stop")
			waitonaboutface += 1 # only do this once
			print("waitonaboutface")
			rospy.sleep(1.5)
...
while not rospy.is_shutdown():
	...
	if t >= nextmove:
		if goalseek and (followpath or goalpose):
			...
			if rosarcmove and goalDistance() > GOALTHRESHOLD:
